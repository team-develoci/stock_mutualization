...
function createAddressProductRelations(quantityLineItemList) {
    ...
    for (var i = 0; i < quantityLineItemList.getChildCount(); i++) {
        ...
        var inventoryID = quantityLineItem.object.inventoryID;

        var uniqueId = 'uniqueProductIdentifier' + productID + productOptionID + inventoryID;
...
function removeNonDefaultShipments(basket) {
    ...
    for (var l = 0; l < shipments.length; l++) {
        ...
        if (!shipment.isDefault() && (shipment.giftCertificateLineItems.length <= 0) && shipment.custom.shipmentType !== 'instore' && (!stockMutualizationEnabled || shipment.ID !== 'sm')) {
